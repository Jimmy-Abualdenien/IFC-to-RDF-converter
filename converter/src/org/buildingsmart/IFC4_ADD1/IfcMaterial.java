package org.buildingsmart.IFC4_ADD1;
import org.buildingsmart.*;
import java.util.*;

/*
 * IFC Java class
/ *
 * The GNU Affero General Public License
 * 
 * Copyright (c) 2014 Jyrki Oraskari (original)
 * Copyright (c) 2014 Pieter Pauwels (modifications - pipauwel.pauwels@ugent.be / pipauwel@gmail.com)
 * 
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as
 * published by the Free Software Foundation, either version 3 of the
 * License, or (at your option) any later version.
 * 
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 * 
 * You should have received a copy of the GNU Affero General Public License
 * along with this program. If not, see <http://www.gnu.org/licenses/>.
 */

public class IfcMaterial extends IfcMaterialDefinition 
{
	// The property attributes
	private IfcLabel Name_of_IfcMaterial;
	private IfcText Description_of_IfcMaterial;
	private IfcLabel Category_of_IfcMaterial;
	// The inverse attributes
	private List<IfcMaterialDefinitionRepresentation> HasRepresentation= new ArrayList<IfcMaterialDefinitionRepresentation>();
	private List<IfcMaterialRelationship> IsRelatedWith= new ArrayList<IfcMaterialRelationship>();
	private List<IfcMaterialRelationship> RelatesTo= new ArrayList<IfcMaterialRelationship>();


	// Getters and setters of properties
	public IfcLabel getName_of_IfcMaterial() {
		return Name_of_IfcMaterial;
	}

	public void setName_of_IfcMaterial(IfcLabel value){
		this.Name_of_IfcMaterial=value;
	}

	public IfcText getDescription_of_IfcMaterial() {
		return Description_of_IfcMaterial;
	}

	public void setDescription_of_IfcMaterial(IfcText value){
		this.Description_of_IfcMaterial=value;
	}

	public IfcLabel getCategory_of_IfcMaterial() {
		return Category_of_IfcMaterial;
	}

	public void setCategory_of_IfcMaterial(IfcLabel value){
		this.Category_of_IfcMaterial=value;
	}

	// Getters and setters of inverse values

	public List<IfcMaterialDefinitionRepresentation> getHasRepresentation() {
		return HasRepresentation;
	}

	public void setHasRepresentation(IfcMaterialDefinitionRepresentation value){
		this.HasRepresentation.add(value);
	}

	public List<IfcMaterialRelationship> getIsRelatedWith() {
		return IsRelatedWith;
	}

	public void setIsRelatedWith(IfcMaterialRelationship value){
		this.IsRelatedWith.add(value);
	}

	public List<IfcMaterialRelationship> getRelatesTo() {
		return RelatesTo;
	}

	public void setRelatesTo(IfcMaterialRelationship value){
		this.RelatesTo.add(value);
	}

}
